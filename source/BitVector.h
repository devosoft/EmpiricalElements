...
namespace emp {
  ...
  private:
    ...
    field_t * bit_set;                                      ///< What is the status of each bit?
    ...
    constexpr static size_t FieldID(const size_t index)  { return index / FIELD_BITS; }
  ...
  public:
    ...
    std::vector<size_t> GetOnes() const {
      ...
      std::vector<size_t> out_set(CountOnes());
      size_t cur_pos = 0;
